...
class WarpAnalyser():
    ...
    def warps_per_type():
        ...
        for g_id in GachaType.objects.all():
            ...
            types[f'{g_id.name}'] = {'pity': pity, 'warranted': warranted, 'wr': winrate, 'c': amount, 'last_win': WarpSerializer(last_win).data if last_win else None, 'max_pity': max_pity, 'jade': jade, 'id': g_id.id}
        return types
...
def getLCPath(name: str):
    ...
    if r.status_code == 200:
        ...
        for lc in data:
            if lc['name'] == f'{name}s' if name in PLURALS else name:
                path = lc['path']
                return path
    else:
